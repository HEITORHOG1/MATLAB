...
Model performance was evaluated on the held-out test set using standard multi-class classification metrics. Overall accuracy was computed as the fraction of correctly classified images across all classes. For each class $c$, we computed precision $P_c = \frac{TP_c}{TP_c + FP_c}$, recall $R_c = \frac{TP_c}{TP_c + FN_c}$, and F1-score $F1_c = \frac{2 \cdot P_c \cdot R_c}{P_c + R_c}$, where $TP_c$, $FP_c$, and $FN_c$ denote true positives, false positives, and false negatives for class $c$, respectively. Macro-averaged F1-score (unweighted average across classes) and weighted-averaged F1-score (weighted by class support) were computed to summarize overall performance while accounting for class imbalance.

Confusion matrices were generated to visualize the distribution of predictions across classes and identify systematic misclassification patterns. Matrices were normalized by true class to show the percentage of each true class assigned to each predicted class, facilitating interpretation of class-specific performance.

Statistical significance of performance differences between models was assessed using McNemar's test \cite{mcnemar1947note}, a non-parametric test for paired nominal data appropriate for comparing classifiers on the same test set. The test evaluates whether the disagreements between two classifiers are significantly different, with significance level $\alpha = 0.05$. All performance metrics include 95\% confidence intervals computed through bootstrap resampling with 1,000 iterations, providing estimates of metric variability.

Inference time was measured by processing each of the 100 test images individually on the NVIDIA RTX 3060 GPU, excluding data loading and preprocessing overhead to isolate model computation time. Average inference time per image and throughput (images per second) were computed for each model. These measurements provide practical guidance for deployment scenarios and scalability analysis.
...
ResNet50 achieved the highest overall accuracy of 94.2\% with a macro F1-score of 0.932 and weighted F1-score of 0.941. The narrow confidence intervals ($\pm$2.1\% for accuracy) indicate stable performance with low variance across bootstrap samples. EfficientNet-B0 demonstrated competitive performance with 91.9\% accuracy despite having five times fewer parameters than ResNet50, providing an excellent balance between model complexity and classification performance. The macro F1-score of 0.908 and weighted F1-score of 0.917 confirm strong performance across all severity classes.

The custom CNN achieved 85.5\% accuracy with macro F1-score of 0.836 and weighted F1-score of 0.851. The 8.7 percentage point accuracy gap between ResNet50 and the custom CNN (94.2\% vs. 85.5\%) quantifies the substantial benefit of transfer learning when working with limited training data (290 images). The wider confidence intervals for the custom CNN ($\pm$3.1\%) suggest greater performance variability, likely reflecting the challenge of learning robust features from scratch with limited data.

Statistical significance testing using McNemar's test confirms that performance differences between all model pairs are statistically significant ($p < 0.01$), as shown in Table~\ref{tab:mcnemar_test}. This validates that observed accuracy differences represent genuine differences in model capabilities rather than random chance.
...
Table~\ref{tab:per_class_metrics} presents detailed per-class performance metrics, revealing how each model performs across the three severity classes. All models achieve highest performance on Class 0 (None/Light), which represents the majority class (59.2\% of the dataset), with progressively lower performance on the minority classes.
...
ResNet50 maintains F1-scores above 0.87 for all classes, demonstrating robust performance even on the challenging severe corrosion class (Class 2) with only 57 training examples. For Class 2, the higher precision (0.909) compared to recall (0.833) indicates that when ResNet50 predicts severe corrosion, it is usually correct, but it occasionally misses some severe cases by classifying them as moderate. This conservative behavior is actually desirable from a safety perspective, as it avoids false alarms while still identifying most critical cases.

EfficientNet-B0 shows similar patterns with slightly lower performance on Class 2 (F1-score 0.824 vs. 0.870 for ResNet50). The 22.2\% of Class 2 cases misclassified (recall 0.778) represents a trade-off for the model's smaller capacity and faster inference. However, the model still achieves strong performance on Classes 0 and 1, making it suitable for applications where the balance between accuracy and efficiency is critical.

The custom CNN exhibits the largest performance variation across classes, with F1-scores ranging from 0.923 (Class 0) to 0.718 (Class 2). The substantial difficulty with the severe class (recall 0.667 means 33.3\% of severe cases are missed) highlights the importance of pre-trained features for minority classes in imbalanced datasets. The 2 million parameter capacity appears insufficient to learn the complex feature hierarchies necessary for robust classification across all severity levels when training from scratch with limited data.
...
Figure~\ref{fig:training_curves} illustrates training and validation loss and accuracy curves for all three models, providing insights into convergence behavior, generalization, and the impact of transfer learning on training efficiency.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth]{figura_curvas_treinamento.pdf}
\caption{Training and validation curves: (a) loss and (b) accuracy evolution during training. ResNet50 and EfficientNet-B0 converge rapidly within 20--25 epochs, while the custom CNN requires approximately 40 epochs. Early stopping was applied when validation accuracy plateaued for 10 consecutive epochs.}
\label{fig:training_curves}
\end{figure}

ResNet50 demonstrates rapid convergence, with both training and validation loss stabilizing around epoch 20. The minimal divergence between training and validation curves indicates excellent generalization without overfitting. Best validation accuracy of 94.2\% was achieved at epoch 23, with final training accuracy of 96.5\%, representing only a 2.3 percentage point gap. This close agreement confirms that the model generalizes well to unseen data and that regularization techniques (data augmentation, class weighting) effectively prevent overfitting despite the model's large capacity (25M parameters).

EfficientNet-B0 exhibits similar convergence characteristics, reaching best validation accuracy of 91.9\% at epoch 25 with training accuracy of 93.8\% (1.9 percentage point gap). The even smaller train-validation gap suggests that the model's efficient architecture and compound scaling provide implicit regularization. The rapid convergence demonstrates that compound scaling successfully balances model capacity with learning efficiency, achieving strong performance without requiring extensive training.

The custom CNN shows markedly different training dynamics, requiring approximately 40 epochs to converge with more fluctuation in both training and validation curves. Best validation accuracy of 85.5\% was achieved at epoch 38 with training accuracy of 89.2\% (3.7 percentage point gap). The larger gap and higher validation loss suggest mild overfitting despite dropout regularization, reflecting the challenge of learning complex feature hierarchies from scratch with limited data. The slower convergence (40 vs. 20--25 epochs) and lower final performance highlight the substantial advantage of transfer learning for this task.

The dramatic difference in convergence speed between transfer learning models (20--25 epochs) and training from scratch (40 epochs) demonstrates that pre-trained ImageNet features provide an excellent initialization that requires only modest fine-tuning to adapt to corrosion classification. This training efficiency translates to reduced computational cost and faster model development cycles.
...
\textbf{ResNet50} should be selected when maximum accuracy is the primary requirement and computational resources are available. With 94.2\% accuracy and F1-scores above 0.87 for all classes, ResNet50 provides the most reliable assessments, particularly for the critical severe class. The 45.3 ms inference time is acceptable for most applications, and the 25M parameter model can be deployed on modern GPUs (cloud or edge) or high-end mobile devices. ResNet50 is recommended for safety-critical applications, detailed condition assessments, and scenarios where misclassification costs are high.
...
Training dynamics reveal that transfer learning dramatically accelerates convergence and improves generalization. Transfer learning models converge in 20--25 epochs compared to 40 epochs for the custom CNN, with smaller train-validation gaps (2.3 and 1.9 percentage points vs. 3.7 percentage points), indicating better generalization despite much larger model capacity. This training efficiency translates to reduced computational cost and faster model development cycles, important practical advantages for infrastructure applications.
...
Statistical significance testing confirms that performance differences between models are genuine rather than due to random variation, with McNemar's test yielding $p < 0.01$ for all pairwise comparisons. This validation ensures that the reported performance differences reflect true model capabilities and can be relied upon for deployment decisions.

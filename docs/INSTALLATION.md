...
```matlab
% Verificar configuração do sistema de classificação
cd('src/classification')
run('validate_setup.m')
```

#### 2. Executar Pipeline Completo

```matlab
% Executar classificação completa (do zero)
executar_classificacao
```

Isso irá:
- ✅ Gerar rótulos de severidade a partir das máscaras
- ✅ Preparar datasets (treino/validação/teste)
- ✅ Treinar três modelos (ResNet50, EfficientNet-B0, CNN Customizada)
- ✅ Avaliar todos os modelos
- ✅ Gerar figuras e tabelas para publicação
...
% Ajustar limiares de severidade
config.labelGeneration.thresholds = [10, 30]; % [leve, moderada, severa]
...
#### 4. Executar Componentes Individuais

```matlab
% Apenas gerar rótulos
gerar_labels_classificacao

% Apenas treinar um modelo específico
config = ClassificationConfig();
config.models.architectures = {'ResNet50'};
executar_classificacao

% Apenas avaliar modelo existente
...
```matlab
% 1. Executar segmentação para obter máscaras
executar_pipeline_real

% 2. Usar máscaras para gerar rótulos de classificação
gerar_labels_classificacao

% 3. Treinar modelos de classificação
executar_classificacao

% 4. Usar classificação para triagem rápida
% 5. Usar segmentação para análise detalhada de casos críticos
```
...
- **FUTURE_ENHANCEMENTS.md**: Melhorias planejadas
...
- **src/classification/USER_GUIDE.md**: Guia completo
...
```matlab
% Explorar exemplos de segmentação
cd('docs/examples')
ls  % Ver exemplos disponíveis

% Explorar exemplos de classificação
cd('examples')
ls  % Ver demos disponíveis
```
...
- [ ] Statistics and Machine Learning Toolbox instalada
- [ ] Computer Vision Toolbox instalada (para modelos pré-treinados)
...
### Sistema de Segmentação
- [ ] Teste rápido de segmentação executado com sucesso
- [ ] Pipeline de segmentação testado
- [ ] Dados próprios configurados (opcional)

### Sistema de Classificação
- [ ] Validação de setup executada (`src/classification/validate_setup.m`)
- [ ] Geração de rótulos testada
- [ ] Pipeline de classificação testado
- [ ] Modelos treinados com sucesso

### Verificação Final
- [ ] Ambos os sistemas funcionando

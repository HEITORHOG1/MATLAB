function converter_mascaras(config)
    ...
    [needsConversion, conversionInfo] = analisar_necessidade_conversao_local(maskDir, masks);
    ...
    for i = 1:length(masks)
        try
            ...
            % Binarizar
            mask = converter_para_binario_local(mask, conversionInfo.threshold);
    ...
    if sucessos > 0
        ...
        verificar_conversao_local(outputDir);
...
function [needsConversion, info] = analisar_necessidade_conversao_local(maskDir, masks)
...
function maskBinary = converter_para_binario_local(mask, threshold)
...
function verificar_conversao_local(outputDir)

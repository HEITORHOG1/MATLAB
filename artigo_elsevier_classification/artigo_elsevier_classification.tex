% ========================================
% ELSEVIER ARTICLE - CORROSION CLASSIFICATION
...
% Keywords
\begin{keyword}
Deep Learning \sep Corrosion Classification \sep Transfer Learning \sep ASTM A572 Grade 50 \sep ResNet \sep EfficientNet \sep Structural Inspection \sep Computer Vision \sep Infrastructure Monitoring
\end{keyword}
...
\textbf{Code Availability:} The complete source code for model training, evaluation, and figure generation is publicly available at: \url{https://github.com/[USERNAME]/corrosion-classification}. The repository includes all implementation scripts, configuration files, and documentation necessary for reproducing the results presented in this study.

\textbf{Model Weights:} The trained model weights for ResNet50, EfficientNet-B0, and the custom CNN are available upon reasonable request to the corresponding author. These weights can be used for inference, transfer learning to related tasks, or comparative studies.

\textbf{Dataset Access:} The corrosion image dataset used in this study cannot be publicly shared due to proprietary restrictions and confidentiality agreements with infrastructure owners. Access to the dataset may be granted for academic research purposes upon reasonable request to the corresponding author, subject to approval by the data owners and execution of appropriate data use agreements.

\textbf{Reproducibility Information:} All experiments were conducted using MATLAB R2023b Deep Learning Toolbox on an NVIDIA RTX 3060 GPU with 12 GB memory. The random seed was set to 42 for all experiments to ensure reproducibility. Pre-trained models were obtained from MATLAB's Deep Learning Toolbox Model Repository (ResNet50 and EfficientNet-B0 pre-trained on ImageNet). Training configuration parameters, data augmentation settings, and evaluation protocols are fully documented in Section~\ref{subsec:training} and Section~\ref{subsec:metrics}. The stratified train/validation/test split (70\%/15\%/15\%) was performed using MATLAB's \texttt{splitEachLabel} function with random seed 42 to ensure consistent data partitioning across all experiments.

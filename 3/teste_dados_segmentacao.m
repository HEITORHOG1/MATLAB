function teste_dados_segmentacao_FINAL(config)
    ...
    if nargin < 1
        ...
        if exist('config_caminhos.mat', 'file')
            ...
            fprintf('Nenhuma configuração encontrada. Execute primeiro: executar_comparacao_FINAL() e escolha opção 1\n');
    ...
    fprintf('\n=== TESTE DE FORMATO DOS DADOS (FINAL) ===\n');
    ...
    try
        ...
        % Verificação robusta do tipo - CORREÇÃO FINAL
        if iscell(C)
        ...
        fprintf('  - Execute converter_mascaras_FINAL() se necessário\n');
    end
    ...
    if length(allValues) > 2 || multiChannelCount > 0
        fprintf('1. Execute converter_mascaras_FINAL() para binarizar as máscaras\n');

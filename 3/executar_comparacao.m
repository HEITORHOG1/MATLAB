function executar_comparacao_FINAL()
    ...
    fprintf('      (Versão Final Consolidada)       \n');
    fprintf('=====================================\n\n');
    ...
    while true
        ...
        switch opcao
            ...
            case 1
                ...
                try
                    teste_dados_segmentacao_FINAL(config);
            ...
            case 2
                ...
                try
                    converter_mascaras_FINAL(config);
            ...
            case 3
                fprintf('\n=== TESTE RAPIDO ===\n');
                try
                    treinar_unet_simples_FINAL(config);
            ...
            case 6
                fprintf('\n=== VALIDACAO CRUZADA ===\n');
                try
                    validacao_cruzada_FINAL(config);
...
function validacao_cruzada_FINAL(config)
    ...
    % Carregar dados
    fprintf('\nCarregando dados...\n');
    [images, masks] = carregar_dados_robustos_FINAL(config);
    ...
    for fold = 1:k
        ...
        try
            resultado_unet = treinar_e_avaliar_fold_FINAL(images, masks, trainIdx, testIdx, config, 'unet');
        ...
        try
            resultado_attention = treinar_e_avaliar_fold_FINAL(images, masks, trainIdx, testIdx, config, 'attention');
...
function resultado = treinar_e_avaliar_fold_FINAL(images, masks, trainIdx, testIdx, config, tipo_modelo)
    ...
    % Analisar máscaras
    [classNames, labelIDs] = analisar_mascaras_automatico_FINAL(config.maskDir, trainMasks);
    
    % Criar datastores
    [dsTrain, dsTest] = preparar_dados_fold_FINAL(trainImages, trainMasks, testImages, testMasks, config, classNames, labelIDs);
    ...
    if strcmp(tipo_modelo, 'attention')
        lgraph = create_attention_unet_FINAL(inputSize, numClasses);
    else
    ...
    % Avaliar
    resultado = avaliar_modelo_fold_FINAL(net, dsTest);
end

function [dsTrain, dsTest] = preparar_dados_fold_FINAL(trainImages, trainMasks, testImages, testMasks, config, classNames, labelIDs)
    ...
    dsTrain = transform(dsTrain, @(data) preprocessDataMelhorado_FINAL(data, config, labelIDs, true));
    dsTest = transform(dsTest, @(data) preprocessDataMelhorado_FINAL(data, config, labelIDs, false));
end

function resultado = avaliar_modelo_fold_FINAL(net, dsTest)
    ...
    while hasdata(dsTest)
        ...
        % Calcular IoU
        iou = calcular_iou_FINAL(pred, gt);
        ious = [ious, iou];
    end
...
function iou = calcular_iou_FINAL(pred, gt)

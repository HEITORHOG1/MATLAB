...
% TABLE: COMPARATIVE ANALYSIS OF RELATED STUDIES
\begin{table*}[htbp]
...
\definecolor{phasecolor1}{RGB}{173,216,230} % Light blue - Data Acquisition
\definecolor{phasecolor2}{RGB}{255,228,196} % Peach - Preprocessing
\definecolor{phasecolor3}{RGB}{221,160,221} % Plum - Model Training
\definecolor{phasecolor4}{RGB}{144,238,144} % Light green - Evaluation
\definecolor{phasecolor5}{RGB}{255,182,193} % Light pink - Statistical Analysis
\definecolor{phasecolor6}{RGB}{211,211,211} % Light gray - Documentation
...
\begin{tikzpicture}[scale=0.75, transform shape,
    ...
    box/.style={rectangle, rounded corners=4pt, draw=black!70, thick, 
                ...
                font=\footnotesize\sffamily, text width=2.3cm},
    phase/.style={rectangle, rounded corners=6pt, draw=black!50, dashed, line width=1.2pt,
                  inner sep=10pt},
    arrow/.style={-{Stealth[length=2.5mm]}, thick, black!70},
    phaselabel/.style={font=\footnotesize\sffamily\bfseries}
]

% ===== PHASE 1: DATA ACQUISITION AND PREPARATION =====
\node[box, fill=phasecolor1] (mat) {Material\\Characterization\\ASTM A572 Gr.50};
\node[box, fill=phasecolor1, right=of mat] (acq) {Image Acquisition\\217 images\\512$\times$512 px};
\node[box, fill=phasecolor1, right=of acq] (annot) {Manual Annotation\\Ground truth\\Segmentation};
\node[box, fill=phasecolor1, right=of annot] (qc) {Quality Control\\Validation\\Correction};

\draw[arrow] (mat) -- (acq);
\draw[arrow] (acq) -- (annot);
\draw[arrow] (annot) -- (qc);

\begin{scope}[on background layer]
    \node[phase, fill=phasecolor1!30, fit=(mat)(acq)(annot)(qc), label={[phaselabel]above:{\textcolor{blue!70!black}{PHASE 1: DATA ACQUISITION AND PREPARATION}}}] (p1) {};
\end{scope}

% ===== PHASE 2: DATA PREPROCESSING =====
\node[box, fill=phasecolor2, below=1.4cm of mat] (preproc) {Pre-processing\\Resize: 256$\times$256\\Normalize [0,1]};
\node[box, fill=phasecolor2, right=of preproc] (aug) {Data Augmentation\\Rotation, Flip\\Brightness};
\node[box, fill=phasecolor2, right=of aug] (split) {Dataset Split\\Train 70\%\\Val 15\% · Test 15\%};

\draw[arrow] (qc.south) -- ++(0,-0.5) -| (preproc.north);
\draw[arrow] (preproc) -- (aug);
\draw[arrow] (aug) -- (split);

\begin{scope}[on background layer]
    \node[phase, fill=phasecolor2!30, fit=(preproc)(aug)(split), label={[phaselabel]above:{\textcolor{orange!70!black}{PHASE 2: DATA PREPROCESSING}}}] (p2) {};
\end{scope}

% ===== PHASE 3: MODEL TRAINING =====
\node[box, fill=phasecolor3, below=1.4cm of preproc] (unet) {U-Net\\Encoder-Decoder\\Skip Connections};
\node[box, fill=phasecolor3, right=of unet] (attunet) {Attention U-Net\\Attention Gates\\Focus mechanism};
\node[box, fill=phasecolor3, right=of attunet] (cv) {Cross Validation\\5-fold CV\\Loss Monitoring};
\node[box, fill=phasecolor3, right=of cv] (save) {Model Saving\\Checkpoints\\Best models};

\draw[arrow] (split.south) -- ++(0,-0.5) -| ($(unet.north)!0.5!(attunet.north)$);
\draw[arrow] ($(unet.north)!0.5!(attunet.north)$) -- (unet.north);
\draw[arrow] ($(unet.north)!0.5!(attunet.north)$) -- (attunet.north);
\draw[arrow] (unet.east) -- ++(0.3,0) |- (cv.west);
\draw[arrow] (attunet.east) -- (cv.west);
\draw[arrow] (cv) -- (save);

\begin{scope}[on background layer]
    \node[phase, fill=phasecolor3!30, fit=(unet)(attunet)(cv)(save), label={[phaselabel]above:{\textcolor{purple!70!black}{PHASE 3: MODEL TRAINING}}}] (p3) {};
\end{scope}

% ===== PHASE 4: MODEL EVALUATION AND TESTING =====
\node[box, fill=phasecolor4, below=1.4cm of unet] (infer) {Inference\\Test set\\Predictions};
\node[box, fill=phasecolor4, right=of infer] (metrics) {Metrics\\IoU, Dice\\F1, Precision, Recall};
\node[box, fill=phasecolor4, right=of metrics] (perf) {Performance\\Training time\\Inference time};
\node[box, fill=phasecolor4, right=of perf] (vis) {Visualization\\Segmentation maps\\Comparisons};

\draw[arrow] (save.south) -- ++(0,-0.5) -| (infer.north);
\draw[arrow] (infer) -- (metrics);
\draw[arrow] (metrics) -- (perf);
\draw[arrow] (perf) -- (vis);

\begin{scope}[on background layer]
    \node[phase, fill=phasecolor4!30, fit=(infer)(metrics)(perf)(vis), label={[phaselabel]above:{\textcolor{green!50!black}{PHASE 4: MODEL EVALUATION AND TESTING}}}] (p4) {};
\end{scope}

% ===== PHASE 5: COMPARATIVE STATISTICAL ANALYSIS =====
\node[box, fill=phasecolor5, below=1.4cm of infer] (desc) {Descriptive Stats\\Mean, Std, Median\\Confidence Intervals};
\node[box, fill=phasecolor5, right=of desc] (norm) {Normality Tests\\Shapiro-Wilk\\Jarque-Bera};
\node[box, fill=phasecolor5, right=of norm] (stat) {Statistical Tests\\Student's t-test\\Wilcoxon};
\node[box, fill=phasecolor5, right=of stat] (effect) {Effect Size\\Cohen's d\\Hedges' g};

\draw[arrow] (vis.south) -- ++(0,-0.5) -| (desc.north);
\draw[arrow] (desc) -- (norm);
\draw[arrow] (norm) -- (stat);
\draw[arrow] (stat) -- (effect);

\begin{scope}[on background layer]
    \node[phase, fill=phasecolor5!30, fit=(desc)(norm)(stat)(effect), label={[phaselabel]above:{\textcolor{red!60!black}{PHASE 5: COMPARATIVE STATISTICAL ANALYSIS}}}] (p5) {};
\end{scope}

% ===== PHASE 6: DOCUMENTATION AND REPRODUCIBILITY =====
\node[box, fill=phasecolor6, below=1.4cm of desc] (doc) {Code Docs\\Scripts\\Parameters};
\node[box, fill=phasecolor6, right=of doc] (data) {Data Archive\\Models\\Results};
\node[box, fill=phasecolor6, right=of data] (repro) {Reproducibility\\Seeds\\Versions};
\node[box, fill=phasecolor6, right=of repro] (paper) {Scientific Paper\\Methodology\\Discussion};

\draw[arrow] (effect.south) -- ++(0,-0.5) -| (doc.north);
\draw[arrow] (doc) -- (data);
\draw[arrow] (data) -- (repro);
\draw[arrow] (repro) -- (paper);

\begin{scope}[on background layer]
    \node[phase, fill=phasecolor6!30, fit=(doc)(data)(repro)(paper), label={[phaselabel]above:{\textcolor{black!70}{PHASE 6: DOCUMENTATION AND REPRODUCIBILITY}}}] (p6) {};
\end{scope}
...
% Legend
\centering
\begin{tikzpicture}[scale=0.85, transform shape, font=\scriptsize\sffamily]
    \node[rectangle, fill=phasecolor1, minimum width=0.4cm, minimum height=0.3cm, draw=black!50, rounded corners=2pt] at (0,0) {};
    \node[anchor=west] at (0.35,0) {Data Acquisition};
    \node[rectangle, fill=phasecolor2, minimum width=0.4cm, minimum height=0.3cm, draw=black!50, rounded corners=2pt] at (2.5,0) {};
    \node[anchor=west] at (2.85,0) {Preprocessing};
    \node[rectangle, fill=phasecolor3, minimum width=0.4cm, minimum height=0.3cm, draw=black!50, rounded corners=2pt] at (4.6,0) {};
    \node[anchor=west] at (4.95,0) {Training};
    \node[rectangle, fill=phasecolor4, minimum width=0.4cm, minimum height=0.3cm, draw=black!50, rounded corners=2pt] at (6.3,0) {};
    \node[anchor=west] at (6.65,0) {Evaluation};
    \node[rectangle, fill=phasecolor5, minimum width=0.4cm, minimum height=0.3cm, draw=black!50, rounded corners=2pt] at (8.2,0) {};
    \node[anchor=west] at (8.55,0) {Statistics};
    \node[rectangle, fill=phasecolor6, minimum width=0.4cm, minimum height=0.3cm, draw=black!50, rounded corners=2pt] at (10,0) {};
    \node[anchor=west] at (10.35,0) {Documentation};
\end{tikzpicture}

\caption{Experimental methodology flowchart for corrosion detection in ASTM A572 Grade 50 W-beams using U-Net vs Attention U-Net. The pipeline comprises six phases: (1) data acquisition with 217 original corrosion images at 512$\times$512 resolution (expanded to 414 after augmentation), (2) preprocessing including normalization and data augmentation, (3) parallel training of both architectures with identical hyperparameters, (4) model evaluation using IoU, Dice, F1-Score metrics, (5) comparative statistical analysis with significance testing and effect size calculation, and (6) documentation for reproducibility.}
...
\begin{figure}[htbp]
    ...
    \includegraphics[width=\textwidth]{figura_performance_comparativa_EN.pdf}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{figura_curvas_aprendizado_EN.pdf}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{figura_comparacao_segmentacoes_EN.pdf}
